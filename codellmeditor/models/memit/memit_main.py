...
def execute_memit(
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        LOG.debug(f"Writing {layer_ks.size(1)} key/value pair(s) into layer {layer}")
        ...
        targets = zs - cur_zs
        LOG.debug(f"z error {torch.linalg.norm(targets, dim=0).mean()}")
        ...
        LOG.debug(f"orig norm { torch.linalg.norm(weights[weight_name])}")
...
def get_context_templates(model, tok):
    ...
    if CONTEXT_TEMPLATES_CACHE is None:
        ...
        LOG.info(f"Cached context templates {CONTEXT_TEMPLATES_CACHE}")
