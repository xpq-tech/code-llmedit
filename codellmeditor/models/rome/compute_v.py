...
def compute_v(
    ...
    LOG.debug("Computing right vector (v)")
    ...
    rewriting_prompts, kl_prompts = [
        context.replace(PADD_STR, request["prompt"]) + ' ' + tok.decode(target_ids[:-1])
        for context in context_templates
    ], [request["prompt"]]
    all_prompts = rewriting_prompts + kl_prompts

    input_tok = tok(
        [prompt.replace(PADD_STR, request["subject"]) for prompt in all_prompts],
        return_tensors="pt",
        padding=True,
    ...
    vanilla_input_prompts = [
        context.replace(PADD_STR, request["prompt"]).replace(PADD_STR, request['subject'])
        for context in context_templates
    ] + [request["prompt"].replace(PADD_STR, request["subject"])]
    lookup_idxs = [
    ...
    LOG.debug(f"Rewrite layer is {layer}")
    LOG.debug(f"Tying optimization objective to {loss_layer}")
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        if cur_layer == hparams.mlp_module_tmp.format(layer):
            ...
            if target_init is None:
                LOG.debug("Recording initial value of v*")
    ...
    LOG.debug(f"Delta norm: {(target - cur_output).norm().item()}")
    LOG.debug(
        f"Change in target norm: {target_init.norm().item()} to {target.norm().item()} => {(target.norm() - target_init.norm()).item()}"
    )
    LOG.debug(f"Division Factor: {torch.dot(cur_input, left_vector).item()}")
    LOG.debug(f"Right vector norm: {right_vector.norm()}")
...
def find_fact_lookup_idx(
    ...
    sentence = prompt.replace(PADD_STR, subject)
    if verbose:
        LOG.debug(
