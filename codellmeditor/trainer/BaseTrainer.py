...
class BaseTrainer:
    def __init__(self, config, train_set: Dataset, val_set: Dataset):
        ...
        make_logs(f"logs/MALMEN_Train_{config.model_name.split('/')[-1]}_{time_now}.log")
        ...
        if any(supported_name in self.config.model_name for supported_name in ["CodeLlama-7b-hf", "CodeQwen1.5-7B", "stable-code-3b"]):
        ...
        model_dir = os.path.join(config.results_dir, f"models_{train_set.data_name}", config.alg)
        ...
        self.save_path = f"{model_dir}/{safe_model_name}_{train_set.data_dir.split('/')[1]}"
