...
class MALMEN(EditableModel):
    ...
    def __init__(
        ...
        self.shift = False
        if any(supported_name in config.model_name.lower() for supported_name in ["llama", "qwen", "stable-code"]):
            self.shift = True
    ...
    def cache(self, batch) -> Dict[int, Dict[int, Dict[str, torch.Tensor]]]:
        ...
        for idx, t in enumerate(batch):
            with TracerDict(
                ...
                logits = self.model(input_ids=t['input_ids'], attention_mask=t['attention_mask'])["logits"]
                cross_entropy(logits, t["labels"], self.shift).backward() # Shift=True Next Token Prediction
